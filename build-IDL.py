...
class Builder:
    ...
    def build_executable(self, clean_build_first, concurrency):
        ...
        buildenv = os.environ
        buildenv["IDLDEPENDENCIES"] = self.depend_path
...
def build_installer(aeb_platforms=[], aeb_output=None,
    ...
    manifest = dict()
    version_info = read_version_h()
    manifest["version"] = version_info["IDL_VERSION_NUMBER"][1:-1]
    manifest["name"] = version_info["IDL_NAME"][1:-1]
    manifest["description"] = version_info["IDL_NAME_LONG"][1:-1]
    aebl_platform_str = ""
...
def main(args):
    ...
    builder = None
    try:
        opticks_depends = os.environ.get("IDLDEPENDENCIES", None)
