...
def build_installer(aeb_platforms=[], aeb_output=None, depend_path=None,
    ...
    metadata[manifest[PF_AEBL + "targetApplication"][0]][PF_AEBL + "minVersion"] = [min_version]
    metadata[manifest[PF_AEBL + "targetApplication"][0]][PF_AEBL + "maxVersion"] = [max_version]
    ...
    try:
        ...
        if options.build_installer:
            ...
            build_installer(aeb_platforms, aeb_output,
                opticks_depends, options.verbosity, opticks_code_dir)
            if options.verbosity > 1:
